...
VassFox_Cvar_2P <-function(Time, State, Pars){
  ...
  with(as.list(c(State, Pars)), {
    ...
    both_preds_eat_C1 <- ( (O_P1_C1 * J_P1 * P1 * C1) / ( (O_P1_C1 * C1) + ((1 - O_P1_C1) * C2) + C_0) ) + ( (O_P2_C1 * J_P2 * P2 * C1) / ( (O_P2_C1 * C1) + ((1 - O_P2_C1) * C2) + C_0) )
    
    both_preds_eat_C2<-( ((1-O_P1_C1) * J_P1 * P1 * C2) / ( (O_P1_C1 * C1) + ((1 - O_P1_C1) * C2) + C_0) ) + ( ((1-O_P2_C1) * J_P2 * P2 * C2) / ( (O_P2_C1 * C1) + ((1 - O_P2_C1) * C2) + C_0) )
    ...
    return(list(c(dP_1, dP_2, dC1, dC2, dR)))
...
State <- c(P1 = 1, P2=1, C1 = 1, C2 = 1, R = 1)
Time <- seq(0, 5000, by = 1)

results <- matrix(data=NA, nrow=5000, ncol=5)
results[1,] <- State
...
for (t in 2:(5000)) {
  ...
  State <- c(P1 = results[t-1,1], P2 = results[t-1,2],C1 = results[t-1,3], C2 = results[t-1,4], R = results[t-1,5])
  VF_out <- as.data.frame(ode(func = VassFox_Cvar_2P, y = State, parms = pars, times = seq(0,1)), 
                          events = list(func = eventfun))
  
  results[t,] <- c(VF_out[2,2],VF_out[2,3], VF_out[2,4], VF_out[2,5], VF_out[2,6])
  
}

dat<- data.frame(pred1=results[,1],pred2=results[,2],con1=results[,3],con2=results[,4],res=results[,5])
d<-melt(dat)
